...
|---|---|---|
| `Header` | `components/app/Header.tsx` | Fixed top bar (`h-16`), fullscreen menu overlay + breadcrumbs. `backdrop-blur`. Nav: Home, About, Projects, Notes, Bookshelf. Layout main uses `pt-16` to offset. |
| `Footer` | `components/app/Footer.tsx` | `h-10`. Live Tokyo clock + theme toggle. Hidden on `/projects/[slug]`. |
